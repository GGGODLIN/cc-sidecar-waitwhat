...
For programs, pass `--session-id <UUID> --json`: the UUID names the session directly instead of the reordering `-s` list, and stdout carries exactly one JSON object — `answer`, `cached`, `source` and `key` on success, `{"ok": false, "error": ...}` on failure. Collie's phone "plain / lost" buttons call it this way.
